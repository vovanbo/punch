...
def test_integer_version_part_init_with_integer():
    vp = IntegerVersionPart(name='major', value=4)
    assert vp.value == 4
...
def test_integer_version_part_init_with_none():
    vp = IntegerVersionPart(name='major', value=None)
    assert vp.value == 0


def test_integer_version_part_init_without_value():
    vp = IntegerVersionPart(name='major')
    assert vp.value == 0


def test_integer_version_part_init_with_start_value():
    vp = IntegerVersionPart(name='major', start_value=1)
    assert vp.value == 1
    vp.inc()
    vp.reset()
    assert vp.value == 1
...
def test_integer_version_part_with_start_value_copy():
    vp = IntegerVersionPart(name='major', start_value=1, value='4')
    nvp = vp.copy()
...
def test_valuelist_version_part_init_with_allowed_value():
    vp = ValueListVersionPart(name='major', allowed_values=[0, 2, 4, 6, 8],
                              value=0)
    assert vp.value == '0'
...
def test_valuelist_version_part_init_with_not_allowed_value():
    with pytest.raises(ValueError):
        ValueListVersionPart(name='major', allowed_values=[0, 2, 4, 6, 8],
                             value=1)
...
def test_valuelist_version_part_increase_from_last():
    vp = ValueListVersionPart('major', [0, 2, 4, 6, 8], value=8)
    vp.inc()
    assert vp.value == '0'
...
def test_valuelist_version_part_increase_with_non_numerical_values():
    vp = ValueListVersionPart(
        'major', [0, 'alpha', 'beta', 'rc1', 'rc2', 1], value=0
    )
    vp.inc()
...
def test_valuelist_version_part_reset():
    vp = ValueListVersionPart('major', [0, 2, 4, 6, 8], value=4)
    vp.reset()
    assert vp.value == '0'
...
def test_get_integer_version_part_from_full_dict():
    ...
    vp = VersionPart.factory(**input_dict)
